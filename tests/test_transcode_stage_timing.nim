...
#
# Decode HEVC/H.265, convert decoded I420 into OwnedRGBXFrame, draw a basic
...
# same synchronous structure as test_transcode_hevc_to_h264_mp4_rgbx_basic so the
...
proc usage() =
  echo "usage:"
  echo "  test_transcode_stage_timing <input> <output.mp4> [decoder] [encoder] [fps] [bitrate] [maxFrames]"
  echo ""
  echo "example:"
  echo "  ./test_transcode_stage_timing bbb_h265.mp4 out_timing.mp4 hevc_v4l2m2m h264_v4l2m2m 30 2000000 300"
...
proc printTimingSummary(timing: PipelineTiming; frameCount: int; packets: int) =
  ...
  printStage("copyI420ToRGBX", timing.copyI420ToRGBX, frameCount)
...
proc readFrameIntoRgbxTimed(
  ...
  eof = false
  timeVoid(timing.copyI420ToRGBX):
    checkVoid(copyI420ToRGBX(read.frame, rgbx))
...
proc main() =
  ...
  timeVoid(timing.copyI420ToRGBX):
    checkVoid(copyI420ToRGBX(firstRead.frame, rgbx))
  ...
  while maxFrames <= 0 or decodedFrames < maxFrames:
    ...
    var eof = false
    readFrameIntoRgbxTimed(decoder, rgbx, timing, eof)
    if eof:
