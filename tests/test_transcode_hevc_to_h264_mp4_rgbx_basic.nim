...
#
# Decode HEVC/H.265, convert decoded I420 into OwnedRGBXFrame, draw a small
# basic overlay, convert RGBX back to encoder-owned I420, encode as H.264, and
# mux into MP4.
#
# This test does not depend on Pixie.  It validates the RGBX detour that Pixie
# or another overlay backend can later use.
...
proc main() =
  ...
  logStep("converting first frame to owned RGBX")
  ...
  while maxFrames <= 0 or decodedFrames < maxFrames:
    ...
    var eof = false
    readFrameIntoRgbx(decoder, rgbx, eof)
    if eof:
